...
async function main() {
  ...
  console.log(
    `  Scallop Deleverage Strategy - ${DRY_RUN_ONLY ? "DRY RUN" : "EXECUTION"} (Plain Transaction)`,
  );
  console.log("═".repeat(60));
  ...
  console.log(`   Protocol pkg: ${coreAddresses.protocolPkg.slice(0, 16)}...`);
  ...
  try {
    ...
    console.log(`\n📊 Estimated Returns:`);
    console.log(`  USDC profit: ~${formatUnits(estimatedProfit, 6)} USDC`);
    ...
    if (DRY_RUN_ONLY) {
      ...
      if (dryRunResult.effects.status.status === "success") {
        ...
        console.log(
          `  Estimated USDC profit: ~${formatUnits(estimatedProfit, 6)} USDC`,
        );
        console.log("─".repeat(60));
      } else {
