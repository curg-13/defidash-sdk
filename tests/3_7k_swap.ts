...
async function main() {
  ...
  const [metadataIn, metadataOut] = await Promise.all([
    client.getCoinMetadata({ coinType: coinTypeIn }),
    client.getCoinMetadata({ coinType: coinTypeOut }),
  ]);

  const decimalsIn = metadataIn?.decimals ?? 9;
  const decimalsOut = metadataOut?.decimals ?? 9;
  const symbolIn = metadataIn?.symbol ?? "UNKNOWN";
  const symbolOut = metadataOut?.symbol ?? "UNKNOWN";

  console.log(
    `\nSwapping ${formatUnits(
      amountIn,
      decimalsIn
    )} ${symbolIn} (${amountIn} units)`
  );
  ...
  try {
    ...
    console.log(
      `Best Quote found: ${formatUnits(
        rawOut,
        decimalsOut
      )} ${symbolOut} (${rawOut} units).`
    );
