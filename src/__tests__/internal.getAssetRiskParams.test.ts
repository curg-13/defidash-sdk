/**
 * Internal: protocol.getAssetRiskParams()  [Integration]
 *
 ...
 *
 * Used by:
 *   - sdk.previewLeverage()         → ltv, liquidationThreshold, maxMultiplier
 *   - sdk.findBestLeverageRoute()   → maxMultiplier for safe multiplier calc
 *
 * Validated fields:
 *   - ltv (0 < ltv < 1)
 *   - liquidationThreshold (0 < threshold <= 1)
 *   - liquidationBonus (0 < bonus < 1)
 *   - maxMultiplier = 1 / (1 - ltv)
 *
 * Requires: .env with SUI_FULLNODE_URL (or defaults to public mainnet)
 *
 * Run: npx vitest run src/__tests__/internal.getAssetRiskParams.test.ts
 */
