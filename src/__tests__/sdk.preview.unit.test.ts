...
vi.mock("../utils/coin", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../utils/coin")>();
  return {
    ...actual,
    getDecimals: vi.fn().mockReturnValue(9),
  };
});
...
describe("previewLeverage (unit)", () => {
  ...
  it("throws on swap quote failure", async () => {
    const deps = createMockDeps({
      swapClient: {
