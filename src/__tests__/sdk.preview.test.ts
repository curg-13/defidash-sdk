/**
 * SDK Method: sdk.previewLeverage()  [Integration]
 *
 * Verifies all output fields across protocols with live mainnet data.
 *
 * Internal dependencies (tested separately):
 *   - protocol.getAssetRiskParams()  → see internal.getAssetRiskParams.test.ts
 *   - protocol.getAssetApy()         → see internal.getAssetApy.test.ts
 *   - ScallopFlashLoanClient.fetchFlashLoanFeeRate()
 *   - 7k Protocol swap quote (live slippage)
 *
 * Verified output fields:
 *   - Position metrics (equity, flash loan, LTV, liquidation price)
 *   - APY breakdown (supply base/reward, borrow gross/rebate/net)
 *   - Flash loan fee (on-chain from Scallop)
 *   - Swap slippage (from live 7k quote)
 *
 * Unit test counterpart: sdk.preview.unit.test.ts
 *
 * Run: npx vitest run src/__tests__/sdk.preview.test.ts
 */
...
describe('previewLeverage Integration', () => {
  ...
  describe('Scallop', () => {
    ...
    it('XBTC 2x – BTC leverage', async () => {
      ...
      assertBaseFields(p, 2.0);
      // Risk params: Scallop XBTC LTV varies (may fallback to 50% if not a native collateral)
      expect(p.assetLtv).toBeGreaterThanOrEqual(0.4);
      expect(p.assetLtv).toBeLessThanOrEqual(0.85);
      expect(p.liquidationThreshold).toBeGreaterThanOrEqual(p.assetLtv);
      // XBTC price >$50k → liquidation price should be high
